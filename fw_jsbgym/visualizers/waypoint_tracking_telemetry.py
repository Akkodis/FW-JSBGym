...
def animate(i, axis, tele_file) -> None:
    ...
    if df.index.size > 0:
        ...
        # axis[2, 1].plot(tsteps, aileron_cmd, label='aileron_cmd' if not np.isnan(np.sum(aileron_cmd)) else '')
        # axis[2, 1].plot(tsteps, elevator_cmd, label='elevator_cmd' if not np.isnan(np.sum(elevator_cmd)) else '')
        # axis[2, 1].plot(tsteps, throttle_cmd, label='throttle_cmd' if not np.isnan(np.sum(throttle_cmd)) else '')
        axis[2, 1].plot(tsteps, aileron_pos_norm, label='aileron_pos_norm' if not np.isnan(np.sum(aileron_pos_norm)) else '')
        axis[2, 1].plot(tsteps, elevator_pos_norm, label='elevator_pos_norm' if not np.isnan(np.sum(elevator_pos_norm)) else '')
        axis[2, 1].plot(tsteps, throttle_pos, label='throttle_pos' if not np.isnan(np.sum(throttle_pos)) else '')
        axis[2, 1].set_title('commands')
        axis[2, 1].set_xlabel("timestep")
        axis[2, 1].set_ylabel("commands [-]")
        axis[2, 1].legend()
        axis[2, 1].grid()
