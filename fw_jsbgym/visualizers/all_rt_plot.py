...
def animate(i, axis, args) -> None:
    ...
    pitch_ref = data['pitch_ref']
    roll_ref = data['roll_ref']
    ...
    turb_north = data['turb_north_mps']
    turb_east = data['turb_east_mps']
    turb_down = data['turb_down_mps']
    ...
    if args.scale:
        ...
        axis[0, 2].set_zlim(alt.iat[-1]-2000, alt.iat[-1]+200)
    ...
    pitch_cmd_plt, = axis[1, 0].plot(tsteps, pitch_ref, label='pitch_ref')
    ...
    roll_cmd_plt, = axis[1, 1].plot(tsteps, roll_ref, label='roll_ref')
    ...
    axis[2, 2].plot(tsteps, turb_north, label='turb_north')
    axis[2, 2].plot(tsteps, turb_east, label='turb_east')
    axis[2, 2].plot(tsteps, turb_down, label='turb_down')
    axis[2, 2].set_title('turbulence linear velocities (m/s)')
    axis[2, 2].legend()
