...
class AttitudeControlTask(Task, ABC):
    ...
    state_vars: Tuple[BoundedProperty, ...] = (
        ...
        prp.airspeed_err, prp.roll_err, prp.pitch_err
    )
    ...
    target_state_vars: Tuple[BoundedProperty, ...] = (
        prp.target_airspeed_kts,
        prp.target_roll_rad, prp.target_pitch_rad
    )

    telemetry_state_vars: Tuple[BoundedProperty, ...] = (
    ...
    def __init__(self, aircraft_id: str, fdm_freq: float, flight_data_logfile: str = "data/flight_data.csv", episode_time_s: float = DEFAULT_EPISODE_TIME_S):
        ...
        self.telemetry_fieldnames: Tuple[str, ...] = tuple([prop.get_legal_name() for prop in self.telemetry_state_vars])
        ...
        self.telemetry: Tuple(BoundedProperty, ...) = (
            ...
            prp.p_radps, prp.q_radps, prp.r_radps, prp.airspeed_kts,
            prp.throttle_cmd, prp.elevator_cmd, prp.aileron_cmd,
            prp.target_airspeed_kts, prp.target_pitch_rad, prp.target_roll_rad,
            prp.airspeed_err, prp.pitch_err, prp.roll_err
        )
    ...
    def reset_target_state(self, sim: Simulation):
        # reset task class attributes with initial conditions
        sim[prp.target_airspeed_kts]: float = sim[prp.initial_airspeed_kts]
        sim[prp.target_roll_rad]: float = sim[prp.initial_roll_rad]
        sim[prp.target_pitch_rad]: float = sim[prp.initial_pitch_rad]
    ...
    def flight_data_logging(self, sim: Simulation):
        ...
        with open(self.flight_data_logfile, 'a') as csv_file:
            ...
            info: dict[str, float] = {}
            for fieldname, prop in zip(self.telemetry_fieldnames, self.telemetry_state_vars):
