...
def train(cfg: OmegaConf):
    ...
    try:
        print(f"Seed provided, using seed from config {cfg.rl_algo.PPO.seed}")
    ...
    run_name = f"ppo_{cfg_rl.exp_name}_{cfg_rl.seed}_{date_time}"
    ...
    if cfg_rl.track:
        wandb.init(
            project=cfg_rl.wandb_project_name,
            entity=cfg_rl.wandb_entity,
    ...
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % ("\n".join([f"|{key}|{value}|" for key, value in vars(cfg_rl).items()])),
    )

    # TRY NOT TO MODIFY: seeding
    random.seed(cfg_rl.seed)
    np.random.seed(cfg_rl.seed)
    torch.manual_seed(cfg_rl.seed)
    torch.backends.cudnn.deterministic = cfg_rl.torch_deterministic

    device = torch.device("cuda" if torch.cuda.is_available() and cfg_rl.cuda else "cpu")
    ...
    unwr_envs = [envs.envs[i].unwrapped for i in range(cfg_rl.num_envs)]
    ...
    optimizer = optim.Adam(agent.parameters(), lr=cfg_rl.learning_rate, eps=1e-5)
    trim_point: TrimPoint = TrimPoint(aircraft_id='x8')
    if "NoVa" in cfg_rl.env_id or "Vanilla" in cfg_rl.env_id:
    ...
    obs = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs) + envs.single_observation_space.shape).to(device)
    obs_t1 = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs) + envs.single_observation_space.shape).to(device)
    actions = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs) + envs.single_action_space.shape).to(device)
    logprobs = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs)).to(device)
    rewards = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs)).to(device)
    terminateds = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs)).to(device)
    truncateds = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs)).to(device)
    values = torch.zeros((cfg_rl.num_steps, cfg_rl.num_envs)).to(device)
    ...
    next_terminated = torch.zeros(cfg_rl.num_envs).to(device)
    num_updates = cfg_rl.total_timesteps // cfg_rl.batch_size

    # Generate a reference sequence and sample the first steps
    refSeqs = [RefSequence(num_refs=3, min_step_bound=600, max_step_bound=700) for _ in range(cfg_rl.num_envs)]
    for _ in range(cfg_rl.num_envs):
    ...
    roll_refs = np.ones(cfg_rl.num_envs) * roll_ref
    pitch_refs = np.ones(cfg_rl.num_envs) * pitch_ref
    a = b = 0.70
    ...
    for update in tqdm(range(1, num_updates + 1)):
        ...
        if cfg_rl.anneal_lr:
            ...
            lrnow = frac * cfg_rl.learning_rate
            optimizer.param_groups[0]["lr"] = lrnow
        
        # save checkpoints periodically
        if cfg_rl.save_cp and update % 8 == 0:
            run_name = f"ppo_{cfg_rl.exp_name}_cp{global_step}_{cfg_rl.seed}_{date_time}"
            save_model(save_path, run_name, agent, envs.envs[0], cfg_rl.seed)
        ...
        for step in range(0, cfg_rl.num_steps):
            ...
            for env_i, done in enumerate(dones):
                if done:
                    ...
                    if cfg_rl.ref_sampler == "beta":
                        if cfg_rl.cst_beta is not None: # sample from beta with constant params
                            roll_refs[env_i] = np.random.beta(cfg_rl.cst_beta, cfg_rl.cst_beta) * roll_limit*2 - roll_limit
                            pitch_refs[env_i] = np.random.beta(cfg_rl.cst_beta, cfg_rl.cst_beta) * pitch_limit*2 - pitch_limit
                            print(f"Sampled from beta with constant params {cfg_rl.cst_beta}")
                        else:
        ...
        with torch.no_grad():
            ...
            lastgaelam = 0
            for t in reversed(range(cfg_rl.num_steps)):
                if t == cfg_rl.num_steps - 1:
                    nextnonterminal = 1.0 - next_terminated
                    nextvalues = next_value
                else:
                    nextnonterminal = 1.0 - terminateds[t + 1]
                    nextvalues = values[t + 1]
                delta = rewards[t] + cfg_rl.gamma * nextvalues * nextnonterminal - values[t]
                advantages[t] = lastgaelam = delta + cfg_rl.gamma * cfg_rl.gae_lambda * nextnonterminal * lastgaelam
        ...
        b_inds = np.arange(cfg_rl.batch_size)
        clipfracs = []
        for epoch in range(cfg_rl.update_epochs):
            np.random.shuffle(b_inds)
            for start in range(0, cfg_rl.batch_size, cfg_rl.minibatch_size):
                end = start + cfg_rl.minibatch_size
                ...
                with torch.no_grad():
                    ...
                    clipfracs += [((ratio - 1.0).abs() > cfg_rl.clip_coef).float().mean().item()]
                ...
                pg_loss2 = -mb_advantages * torch.clamp(ratio, 1 - cfg_rl.clip_coef, 1 + cfg_rl.clip_coef)
                pg_loss = torch.max(pg_loss1, pg_loss2).mean()

                # Value loss
                newvalue = newvalue.view(-1)
                if cfg_rl.clip_vloss:
                ...
                b_cmd = torch.zeros((cfg_rl.minibatch_size, 2)).to(device)
                if cfg_rl.env_id not in ["ACNoVaPIDRLAdd-v0", "ACNoVaPIDRL-v0"]:
                ...
                loss = pg_loss - cfg_rl.ent_coef * entropy_loss + v_loss * cfg_rl.vf_coef \
                      + cfg_rl.ts_coef * ts_loss + cfg_rl.ss_coef * ss_loss + cfg_rl.pa_coef * pa_loss

                optimizer.zero_grad()
                loss.backward()
                nn.utils.clip_grad_norm_(agent.parameters(), cfg_rl.max_grad_norm)
                optimizer.step()

            if cfg_rl.target_kl is not None:
                if approx_kl > cfg_rl.target_kl:
                    break
        ...
        writer.add_scalar("losses/value_loss_term", cfg_rl.vf_coef * v_loss.item(), global_step)
        writer.add_scalar("losses/policy_loss", pg_loss.item(), global_step)
        writer.add_scalar("losses/ts", ts_loss.item(), global_step)
        writer.add_scalar("losses/ts_term", cfg_rl.ts_coef * ts_loss.item(), global_step)
        writer.add_scalar("losses/ss", ss_loss.item(), global_step)
        writer.add_scalar("losses/ss_term", cfg_rl.ss_coef * ss_loss.item(), global_step)
        writer.add_scalar("losses/pa", ss_loss.item(), global_step)
        writer.add_scalar("losses/pa_term", cfg_rl.pa_coef * pa_loss.item(), global_step)
        writer.add_scalar("losses/entropy", entropy_loss.item(), global_step)
        writer.add_scalar("losses/entropy_term", cfg_rl.ent_coef * entropy_loss.item(), global_step)
    ...
    if not cfg_rl.no_eval_plot:
        ...
        for step in range(4000):
            ...
            if done:
                ...
                # if cfg_rl.save_best:
                #     if (cfg_rl.env_id == "ACBohn-v0" and r_per_step > -0.20) or \
                #        ((cfg_rl.env_id == "ACBohnNoVa-v0" or cfg_rl.env_id == "ACBohnNoVaIErr-v0") and r_per_step > -0.06):
                #         save_model(save_path, run_name, agent, pe_env, cfg_rl.seed)
                # else:
                #     save_model(save_path, run_name, agent, pe_env, cfg_rl.seed)
                break
