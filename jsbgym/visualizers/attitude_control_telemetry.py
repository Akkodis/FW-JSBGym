...
def animate(i, axis, args) -> None:
    ...
    if df.index.size > 0:
        ...
        if df.index.size > 1:
            ...
            axis[1, 0].fill_between(tsteps, pitch_ref - np.deg2rad(5), pitch_ref + np.deg2rad(5), color='r', alpha=0.2)
            axis[1, 0].set_title('pitch control')
            axis[1, 0].set_xlabel("timestep")
            axis[1, 0].set_ylabel("pitch [rad]")
            axis[1, 0].legend()
            axis[1, 0].grid()
            ...
            axis[1, 1].fill_between(tsteps, roll_ref - np.deg2rad(5), roll_ref + np.deg2rad(5), color='r', alpha=0.2)
            axis[1, 1].set_title('roll control')
            axis[1, 1].set_xlabel("timestep")
            axis[1, 1].set_ylabel("roll [rad]")
            axis[1, 1].legend()
            axis[1, 1].grid()
